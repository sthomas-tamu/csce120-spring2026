...
int main() {
  ...
  try {
    ...
    std::cout << "Error, no filename given for " << msg << std::endl;
    return -1;
  }
  ...
  while (!origIn.eof()) {
    ...
    try {
      ...
      std::cout << "Error, " << msg << ", exiting." << std::endl;
      return -1;
    }
