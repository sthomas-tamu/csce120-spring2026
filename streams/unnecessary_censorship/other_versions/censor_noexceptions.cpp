...
int main() {
  ...
  if (!censorIn.is_open()) {
    std::cout << "Unable to open " << censorWordsFilename << std::endl;
    return -1;
  }
  std::ifstream origIn(originalTextFilename);
  if (!origIn.is_open()) {
    std::cout << "Unable to open " << originalTextFilename << std::endl;
    return -1;
  }
  std::ofstream censorOut(censoredTextFilename);
  if (!censorOut.is_open()) {
    std::cout << "Unable to open " << censoredTextFilename << std::endl;
    return -1;
  }
  ...
  while (!origIn.eof()) {
    std::string line;
    getline(origIn, line);
    if (!origIn.eof() && origIn.fail()) {
      std::cout << "Error, failed to read line from origIn, exiting." << std::endl;
      return -1;
    }
